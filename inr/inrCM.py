...
# ═════════════════════════════════════════════════════════════════════════════
QUALITY_PRESETS = {
    "low":    {"steps": 1000,  "hidden_dim": 128, "num_layers": 4, "omega_0": 30.0, "lr": 2e-4},
    "medium": {"steps": 2000,  "hidden_dim": 256, "num_layers": 5, "omega_0": 30.0, "lr": 1e-4},
    "high":   {"steps": 5000,  "hidden_dim": 512, "num_layers": 6, "omega_0": 30.0, "lr": 5e-5},
    "ultra":  {"steps": 10000, "hidden_dim": 1024,"num_layers": 7, "omega_0": 30.0, "lr": 2e-5},
}
...
class CrossModalSIREN(nn.Module):
    ...
    def __init__(self, in_dim, out_dim_1, out_dim_2,
        ...
        # Two output heads
        self.head_cover  = nn.Sequential(nn.Linear(hidden_dim, out_dim_1), nn.Sigmoid())
        self.head_secret = nn.Sequential(nn.Linear(hidden_dim, out_dim_2), nn.Sigmoid())

    def forward_cover(self, coords):
        return self.head_cover(self.backbone(coords))

    def forward_secret(self, coords):
        return self.head_secret(self.backbone(coords))
...
def load_audio_modality(path, max_samples=22050):
    ...
    meta = {"type": "audio", "N": N, "sr": sr, "path": path,
            "orig_min": float(y.min()), "orig_max": float(y.max())}
    return data, coords, meta
...
# ── Text ──────────────────────────────────────────────────────────────────────
def load_text_modality(path, max_chars=1024):
    with open(path, "r", encoding="utf-8") as f:
        text = f.read()[:max_chars]
    # Encode as normalized char indices
    chars  = [ord(c) / 127.0 for c in text]
    N      = len(chars)
...
def save_text_modality(tensor, meta, out_path):
    arr   = tensor.reshape(-1).clamp(0, 1).cpu().numpy()
    chars = [chr(max(32, min(126, int(round(v * 127))))) for v in arr]
...
def hide(args, device, cfg):
    ...
    # Loss weight — cover slightly more important for imperceptibility
    w_cover  = 0.6
    w_secret = 0.4
...
def main():
    ...
    parser.add_argument("--quality",  type=str, default=None,
                        ...
                        help="Quality preset. Auto-selected if omitted.")
